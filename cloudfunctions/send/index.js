  ...
  try {
        ...
        for (var k = 0; k < arr_xtz_messages.length; k++){
          for (var j = 0; j < arr_xtz_messages[k]["arr_content"][0].length; j++){
              ...
              str_thing4 = current_cycle + ' Cycle 应收到来自' + arr_xtz_messages[k]["arr_content"][1][j] + "的奖励tezos //"      +    arr_xtz_messages[k]["arr_content"][2][j].toFixed(2)
              console.log(arr_xtz_messages[k]["open_id"])
              await cloud.openapi.subscribeMessage.send({
                touser: arr_xtz_messages[k]["open_id"],
